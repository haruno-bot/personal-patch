...
type cqpbclient struct {
	...
	servToken      string
	pluginCallback map[string]func(string)
}
...
func (c *cqpbclient) Initialize(token, servToken string) {
	...
	c.servConn.OnMessage = func(raw []byte) {
		...
		case kcwiki_msgtransfer_protobuf.Websocket_SYSTEM:
			...
			logger.Service.Field("Server Socket").Successf("%s", wsSystem.GetData())
		case kcwiki_msgtransfer_protobuf.Websocket_NON_SYSTEM:
			...
			if c.pluginCallback[wsNonSystem.GetModule()] == nil {
				return
			}
			c.pluginCallback[wsNonSystem.GetModule()](wsNonSystem.GetData())
		}
	}
...
var PbClient = &cqpbclient{
	...
	pluginCallback: make(map[string]func(string)),
}
